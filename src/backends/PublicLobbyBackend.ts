...
function fmtName(player: skeldjs.PlayerData|undefined) {
    ...
    const consoleClr: chalk.Chalk = colours[colour] || colours[skeldjs.Color.Grey];
...
export default class PublicLobbyBackend extends BackendAdapter {
    ...
    async initialize(): Promise<void> {
        ...
        try {
            ...
            this.client.on("client.disconnect", async ev => {
                this.log(LogMode.Info, "Client disconnected: " + (ev.reason === undefined ? "No reason." : (ev.reason + " (" + ev.message + ")")));
            ...
            this.client.on("player.move", ev => {
                if (ev.player?.data) {
                    this.emitPlayerPose(ev.player.data.name, ev.position);
                }
            });

            this.client.on("player.snapto", ev => {
                if (ev.player?.data) {
                    this.log(LogMode.Log, "Got SnapTo for", fmtName(ev.player), "to x: " + ev.position.x + " y: " + ev.position.y);
                    this.emitPlayerPose(ev.player.data.name, ev.position);
                } else {
            ...
            this.client.on("player.setcounter", ev => {
                if (ev.counter === 5) {
            ...
            this.client.on("player.sethost", async ev => {
                ...
                if (ev.player && ev.player.data) {
                    this.log(LogMode.Info, fmtName(ev.player), " is now the host.");
                    this.emitHostChange(ev.player.data.name);
                } else {
            ...
            this.client.on("player.join", ev => {
                ...
                this.log(LogMode.Info, "Player with ID " + ev.player.id + " joined the game.");
            });
            ...
            this.client.on("player.leave", ev => {
                ...
                this.log(LogMode.Log, "Player with ID " + ev.player.id + " left or was removed.");
            });
            ...
            this.client.on("player.syncsettings", ev => {
                if (ev.options.crewmateVision !== this.settings.crewmateVision) {
                    this.settings.crewmateVision = ev.options.crewmateVision;

                    this.log(LogMode.Info, "Crewmate vision is now set to " + ev.options.crewmateVision + ".");
                }
                
                if (ev.options.map !== this.settings.map) {
                    this.settings.map = ev.options.map;

                    this.log(LogMode.Info, "Map is now set to " + skeldjs.GameMap[ev.options.map] + ".");
                }
            ...
            this.client.on("player.setname", ev => {
                if (ev.player?.data) {
                    ...
                    if (ev.player) {
                        this.log(LogMode.Warn, "Name was set for " + ev.player.id + ", but there was no data.");
                    } else {
            ...
            this.client.on("player.setcolor", ev => {
                if (ev.player?.data) {
                    this.log(LogMode.Info, fmtName(ev.player), "set their colour to " + skeldjs.Color[ev.color] + ".");
                    this.emitPlayerColor(ev.player.data.name, ev.color);
                } else {
                    if (ev.player) {
                        this.log(LogMode.Warn, "Color was set for " + ev.player.id + ", but there was no data.");
                    } else {
            ...
            this.client.on("player.callmeeting", async () => {
                ...
                if (ev.component.classname === "MeetingHud") {
                    ...
                    const all_states = [...ev.component.states.values()];
            ...
            this.client.on("meetinghud.votingcomplete", async ev => {
                ...
                if (ev.ejected) {
                    if (ev.ejected.data) {
                        this.emitPlayerFlags(ev.ejected.data.name, PlayerFlag.IsDead, true);
            ...
            this.client.on("player.murder", ev => {
                if (ev.victim && ev.victim.data) {
                    this.log(LogMode.Info, fmtName(ev.player), "murdered", fmtName(ev.victim) + ".");
                    this.emitPlayerFlags(ev.victim.data.name, PlayerFlag.IsDead, true);
                } else {
            ...
            this.client.on("player.entervent", ev => {
                if (ev.player && ev.player.data) {
                    this.log(LogMode.Log, fmtName(ev.player), "entered vent " + this.getVentName(ev.ventid) + ".");
                    this.emitPlayerVent(ev.player.data.name, ev.ventid);
                } else {
            ...
            this.client.on("player.exitvent", ev => {
                if (ev.player && ev.player.data) {
                    this.log(LogMode.Log, fmtName(ev.player), "exited vent " + this.getVentName(ev.ventid) + ".");
                    this.emitPlayerVent(ev.player.data.name, -1);
                } else {
            ...
            this.client.on("security.cameras.join", ev => {
                if (ev.player?.data) {
                    ...
                    this.emitPlayerFlags(ev.player.data.name, PlayerFlag.OnCams, true);
                } else {
            ...
            this.client.on("security.cameras.leave", ev => {
                if (ev.player?.data) {
                    ...
                    this.emitPlayerFlags(ev.player.data.name, PlayerFlag.OnCams, false);
                } else {
            ...
            this.client.on("player.move", ev => {
                ...
                if (ev.player?.data) {
                    this.log(LogMode.Log, fmtName(ev.player), "moved to X: " + ev.position.x + ", Y: " + ev.position.y);
                } else {
        ...
        return new Promise<boolean>(resolve => {
            ...
            this.client.on("component.spawn", function onSpawn(ev) {
                ...
                if (ev.component.classname === "GameData") {
                    ...
                    for (const [ , player ] of ev.component.players) {
    ...
    async initialSpawn(isFinal = false): Promise<ConnectionErrorCode> {
        ...
        try {
            ...
            if (!this.client)
                return ConnectionErrorCode.NoClient;
            await Promise.race([this.client.identify("auproxy", this.client.token), sleep(2000)]);
